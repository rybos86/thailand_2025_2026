...
const AccommodationBox: React.FC<{ leg: TripLeg; className?: string }> = ({ leg, className = '' }) => (
  <div className={`bg-slate-50 p-5 rounded-xl border border-slate-100 ${className}`}>
    ...
    <div className="mb-4">
      ...
      <div className="text-sm text-slate-500 leading-snug mb-3">
        ...
        <div className="flex gap-2 text-xs mb-2">
          {leg.accommodation.checkIn && (
            <div className="bg-white px-2 py-1 rounded border border-slate-200">
              ...
              <span className="font-semibold text-slate-700">{leg.accommodation.checkIn}</span>
            </div>
          )}
          {leg.accommodation.checkOut && (
            <div className="bg-white px-2 py-1 rounded border border-slate-200">
              <span className="text-slate-500">Check-out:</span>{' '}
              <span className="font-semibold text-slate-700">{leg.accommodation.checkOut}</span>
            </div>
          )}
        </div>
      )}
      ...
      {leg.accommodation.phone && (
        <a
          ...
          className="flex items-center gap-2 text-sm text-thailand-600 hover:text-thailand-700 mb-3"
        >
