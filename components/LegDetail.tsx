...
const AccommodationBox: React.FC<{ leg: TripLeg; className?: string }> = ({ leg, className = '' }) => (
  <div className={`bg-slate-50 dark:bg-slate-800 p-5 rounded-xl border border-slate-100 dark:border-slate-700 ${className}`}>
    ...
    <div className="mb-4">
      <div className="font-bold text-slate-800 dark:text-slate-200 text-lg leading-tight mb-1">
        {leg.accommodation.name}
      </div>
      <div className="text-sm text-slate-500 dark:text-slate-400 leading-snug mb-3">
        {leg.accommodation.address}
      </div>

      {/* Check-in/Check-out times */}
      {(leg.accommodation.checkIn || leg.accommodation.checkOut) && (
        <div className="flex gap-2 text-xs mb-2">
          {leg.accommodation.checkIn && (
            <div className="bg-white dark:bg-slate-700 px-2 py-1 rounded border border-slate-200 dark:border-slate-600">
              <span className="text-slate-500 dark:text-slate-400">Check-in:</span>{' '}
              <span className="font-semibold text-slate-700 dark:text-slate-300">{leg.accommodation.checkIn}</span>
            </div>
          )}
          {leg.accommodation.checkOut && (
            <div className="bg-white dark:bg-slate-700 px-2 py-1 rounded border border-slate-200 dark:border-slate-600">
              <span className="text-slate-500 dark:text-slate-400">Check-out:</span>{' '}
              <span className="font-semibold text-slate-700 dark:text-slate-300">{leg.accommodation.checkOut}</span>
            </div>
          )}
        </div>
      )}
      ...
      {/* Phone number */}
      {leg.accommodation.phone && (
        <a
          href={`tel:${leg.accommodation.phone}`}
          className="flex items-center gap-2 text-sm text-thailand-600 dark:text-thailand-400 hover:text-thailand-700 dark:hover:text-thailand-300 mb-3"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
          </svg>
          <span className="font-medium">{leg.accommodation.phone}</span>
        </a>
      )}
    </div>
    ...
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex items-center justify-center w-full py-2 px-4 bg-white dark:bg-slate-700 text-thailand-600 dark:text-thailand-400 border border-thailand-200 dark:border-thailand-700 rounded-lg text-sm font-medium hover:bg-thailand-50 dark:hover:bg-thailand-900/30 hover:border-thailand-300 dark:hover:border-thailand-600 transition-all shadow-sm group/link"
    >
...
export const LegDetail: React.FC<Props> = ({ leg, index, isLast }) => {
  ...
  return (
    <div id={leg.id} className="flex gap-4 md:gap-8 group scroll-mt-32 relative">
      ...
      {!isFirst && (
        <div className="absolute left-5 md:left-7 top-0 h-6 w-0.5 md:w-1 bg-slate-200 dark:bg-slate-700 transform -translate-x-1/2 z-0 hidden md:block" />
      )}
      {!isLast && (
        <div className="absolute left-5 md:left-7 top-6 bottom-0 w-0.5 md:w-1 bg-slate-200 dark:bg-slate-700 transform -translate-x-1/2 z-0 hidden md:block" />
      )}

      {/* Left Column: Marker */}
      <div className="flex flex-col items-center flex-shrink-0 w-10 md:w-14 pt-0 relative z-10 hidden md:flex">
        <div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white dark:bg-slate-800 border-4 border-thailand-500 dark:border-thailand-400 shadow-sm flex items-center justify-center text-thailand-700 dark:text-thailand-300 font-bold text-lg md:text-xl mt-1">
          {index + 1}
        </div>
      </div>
      ...
      <div className="flex-grow pb-12 md:pb-16 min-w-0 mt-1">
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm hover:shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden transition-shadow duration-300">
          ...
          <div className="p-4 md:p-8">
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                <p className="text-slate-700 dark:text-slate-300 text-lg leading-relaxed font-medium border-l-4 border-thailand-200 dark:border-thailand-600 pl-4">
                  {leg.description}
                </p>
                  ...
                  return ContentComponent ? (
                    <div className="bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-700 overflow-hidden">
                      <button
                        ...
                        className="flex items-center justify-between w-full p-4 md:p-6 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group/toggle"
                      >
